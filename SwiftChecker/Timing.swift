...
/**
Returns the `TimeStamp `value in seconds for relative `TimeStamp`s. For absolute `TimeStamp`s, returns the age instead.
*/
...
/**
This is an optional label for the `TimeStamp`. Used only by the description and age properties.
*/
...
/**
Returns a printable description of the current age of the `TimeStamp` (returns description if it's relative).
*/
...
`TimeStamp`s. This makes no sense if both sides are absolute.
*/
public func + (left: TimeStamp, right: TimeStamp) -> TimeStamp! {
	if left.absolute && right.absolute {
		return nil		// wrong combination, boom!
...
public func BenchmarkSerial(comment: String?, times: Int, work: BenchClosure) -> TimeStamp! {
	...
	let total = _BenchmarkSerial(times, work: work)
	return TimeStamp(false, total, comment)
}

/**
This function accepts a repetition count (which should be at least 1000 to be useful),
and a closure to be benchmarked in serial; it produces a relative `TimeStamp` containing
the average execution time for the closure.

This makes no sense if the repetition count is < 1.
*/
public func BenchmarkSerial(times: Int, work: BenchClosure) -> TimeStamp! {
	return BenchmarkSerial(nil, times: times, work: work)
}
...
public func BenchmarkParallel(comment: String?, times: Int, work: BenchClosure) -> TimeStamp! {
	...
	let total = _BenchmarkParallel(times, work: work)
	return TimeStamp(false, total, comment)
}

/**
This function accepts a repetition count (which should be a multiple of 10, and at least 100 to be useful),
and a closure to be benchmarked in parallel; it produces a relative `TimeStamp` containing
the average execution time for the closure.

This makes no sense if the repetition count is < 1.
*/
public func BenchmarkParallel(times: Int, work: BenchClosure) -> TimeStamp! {
	return BenchmarkParallel(nil, times: times, work: work)
}
...
public func ReportTimingData() {
	Print("Timing quantum = \(TimeStamp._format(false,1))")
	let overhead = BenchmarkSerial(1000) {
		return TimeStamp()
	}
	Print("TimeStamp overhead = \(overhead)")
}
...
/**
private concurrent dispatch queue for benchmarking
*/
